...
class KBQueryWorkflow:
    ...
    def _route_after_item_name_confirm(self, state: QueryGraphState) -> str:
        ...
        if state.get("answer"):
            ...
            具体来说：
            1. 多选一（反问用户）：
            - 场景：用户问得较模糊，库中有多个相近实体。
            - 处理：生成反问句作为 answer，例如：“您是想了解以下哪个内容：宽窄巷子、锦里？请再明确一下。”
            - 结果：此时不继续检索文档，直接把这句话发给用户。

            2. 宽泛问题 / 未抽到实体：
            - 不再硬拒绝，answer 为空，继续全库或软过滤检索。
            """
    ...
    def run(self, initial_state: QueryGraphState, stream: bool = False) -> QueryGraphState:
        ...
        :return: stream=False 时返回最终 state；stream=True 时返回必须被 for 消费的生成器
                 （不消费则图不会执行任何节点）
        """
        if not self._compiled_app:
            self.compile()
        if stream:
            # 调用方必须：for event in workflow.run(..., stream=True): ...
