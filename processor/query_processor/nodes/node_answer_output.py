...
from utils.task_utils import set_task_result, get_done_task_list
...
class NodeAnswerOutput(NodeBase):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        # 若 answer 在进入本节点前已存在，说明 node_item_name_confirm 已写入过助手消息，避免重复落库
        if state.get("answer") and not answer_exists:
            logger.info("---写入MongoDB历史记录---")
            self._step_4_write_history(state, image_urls=image_urls)

        # 先标记节点完成并推送 progress（清空 running），再发 final
        # 否则前端收到 final 会立即 es.close()，收不到“进行中已清空”的进度事件
        logger.info("---node_answer_output 节点处理结束---")

        # 阶段五: 流式输出结束，发送 final 事件
