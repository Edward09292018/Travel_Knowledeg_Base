...
class NodeMDImg(BaseNode):
    ...
    def process(self, state: ImportGraphState):
        ...
        if lm_config.backend == "transformers" and lm_config.release_vl_after_use:
            llm_utils.release_vl_model()
    ...
    def _step_1_get_content(self, state: ImportGraphState) -> Tuple[str, Path, Path]:
        ...
        # 4、获取 md_content（上游已写入；无图跳过时也不改写，原样透传即可）
    ...
    def _step_3_generate_summaries(self, doc_stem: str, target_images: List[Tuple[str, str, Tuple[str, str]]]) -> Dict[
        ...
        # 1、仅远程 API 需要限流；本地 transformers 不限
        request_deque = deque()
        use_rate_limit = lm_config.backend == "openai"

        # 2、循环处理图片
        total = len(target_images)
        for idx, (img_file, image_path, context) in enumerate(target_images, start=1):
            # 2.1、速率限制（仅远程）
            if use_rate_limit:
                self._apply_api_rate_limit(request_deque, max_requests=60)

            if lm_config.backend == "transformers":
                self.logger.info(f"开始总结图片 [{idx}/{total}]: {img_file}")
            # 2.2、调用大模型生成图片摘要
            summaries[img_file] = self._summarize_image(image_path, root_folder=doc_stem, image_content=context)
            if lm_config.backend == "transformers":
                self.logger.info(f"图片总结完成 [{idx}/{total}]: {img_file}")
    ...
    def _summarize_image(self, image_path: str, root_folder: str, image_content: Tuple[str, str]) -> str:
        ...
        try:
            chat_model = llm_utils.get_vl_chat()
            messages = [
                {
                    "role": "user",
                    "content": [
