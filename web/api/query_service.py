...
app = FastAPI(
    title="旅游知识库-查询API",
    description="此文档是旅游知识库查询流程的API接口说明"
)
...
def run_query_graph(session_id: str, user_query: str, is_stream: bool = True):
    ...
    try:
        ...
        # 两层流式要分开看：
        # 1) LangGraph stream：按节点推进图，必须 for 消费才会执行
        # 2) 前端 SSE：节点内 push_to_session（进度 / delta），与 1) 并行发生
        # is_stream=True 走真 stream；False 才 invoke（不要对 dict 做 for）
        if is_stream:
            for chunk in workflow.run(init_state, stream=True):
                logger.debug(chunk)
            push_to_session(session_id, SSEEvent.CLOSE, {})
        else:
            workflow.run(init_state, stream=False)
